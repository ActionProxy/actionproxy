...
// These are exact, single-occurrence assertions that prove removed Community
// interfaces stay unavailable. Replacing only the known assertion text keeps a
// second occurrence in the same test file visible to the strict rules.
...
const narrativeRules = [
  ...
  [/\b(?:TODO|TBD|FIXME)\b|\bcoming\s+soon\b|\bplaceholder\s+copy\b/i, 'placeholder copy'],
  [/\/mcp\/chatgpt-work\b|\bACTIONPROXY_CHATGPT_WORK(?:_[A-Z0-9_]+)?\b/i, 'legacy ChatGPT onboarding copy'],
  [/\bNode(?:\.js)?(?:\s+|:\s*)v?20(?:\.\d+)*\b/i, 'unsupported Node 20 guidance'],
];
...
async function verifyManifestArtifact() {
  ...
  if (manifest?.schemaVersion !== 'actionproxy.public-manifest.v1') {
    failures.push('PUBLIC_MANIFEST.json has an unexpected schemaVersion');
  }
  const canonicalRepository = canonicalGitHubRepositoryUrl(manifest?.repository);
  ...
  if (!isSemverTag(manifest?.releaseTag)) {
    failures.push('PUBLIC_MANIFEST.json releaseTag must be a strict v-prefixed semantic version');
  ...
  const configuredRepository = process.env.ACTIONPROXY_PUBLIC_REPOSITORY_URL?.trim();
  if (
    configuredRepository &&
    canonicalGitHubRepositoryUrl(configuredRepository) !== manifest?.repository
  ) {
    failures.push('PUBLIC_MANIFEST.json repository does not match ACTIONPROXY_PUBLIC_REPOSITORY_URL');
  }
  const configuredTag = process.env.ACTIONPROXY_PUBLIC_RELEASE_TAG?.trim();
  if (configuredTag && configuredTag !== manifest?.releaseTag) {
    failures.push('PUBLIC_MANIFEST.json releaseTag does not match ACTIONPROXY_PUBLIC_RELEASE_TAG');
  }
  ...
  for (const [index, entry] of manifest.files.entries()) {
    if (!hasExactKeys(entry, ['path', 'sha256'])) {
      failures.push(`PUBLIC_MANIFEST.json contains non-canonical fields at files[${index}]`);
    }
    if (!isSafeRelativePath(entry?.path) || entry.path === 'PUBLIC_MANIFEST.json') {
      failures.push(`PUBLIC_MANIFEST.json contains an unsafe or reserved path at files[${index}]: ${String(entry?.path)}`);
      continue;
    }
    if (!/^[a-f0-9]{64}$/u.test(entry?.sha256 ?? '')) {
      failures.push(`PUBLIC_MANIFEST.json contains an invalid SHA-256 for ${entry.path}`);
    }
    if (declared.has(entry.path)) {
      failures.push(`PUBLIC_MANIFEST.json contains duplicate path: ${entry.path}`);
      continue;
    }
  ...
  for (const [relativePath, digest] of declared) {
    if (!actualSet.has(relativePath)) {
      failures.push(`PUBLIC_MANIFEST.json references missing file: ${relativePath}`);
      continue;
    }
    try {
      const contents = await fs.readFile(path.join(destination, relativePath));
  ...
  for (const relativePath of actualPaths) {
    if (!declared.has(relativePath)) {
      failures.push(`Unexpected file absent from PUBLIC_MANIFEST.json: ${relativePath}`);
    }
  }
...
async function verifyPackagesAndCommands() {
  ...
  for (const relativePath of packageFiles) {
    try {
      packages.push({
        ...
        manifest: JSON.parse(await fs.readFile(path.join(destination, relativePath), 'utf8')),
        relativePath,
      });
    } catch (error) {
  ...
  const rootPackage = packages.find(({ relativePath }) => relativePath === 'package.json')?.manifest;
  if (!rootPackage) return;
  const actualScripts = Object.keys(rootPackage.scripts ?? {}).sort(comparePaths);
  ...
  if (
    ...
    failures.push(`Root package scripts must match the public allowlist exactly: ${expectedScripts.join(', ')}`);
  }
  if (rootPackage.engines?.node !== '>=22 <25') {
    failures.push('package.json engines.node must support exactly Node 22 and 24');
  }
  if (!/^pnpm@11\.\d+\.\d+$/u.test(rootPackage.packageManager ?? '')) {
    failures.push('package.json packageManager must pin an exact pnpm 11 release');
  }
  ...
  const workspaceNames = new Set(packages.map(({ manifest }) => manifest?.name).filter(Boolean));
  for (const { directory, manifest, relativePath } of packages) {
    ...
    for (const [scriptName, command] of Object.entries(manifest.scripts ?? {})) {
      if (typeof command !== 'string' || command.trim() === '') {
        failures.push(`${relativePath} has an empty or non-string script: ${scriptName}`);
        continue;
      }
      if (/(?:^|:)site(?::|$)|publish|ghcr\.io|ACTIONPROXY_EDITION|VITE_ACTIONPROXY_EDITION|marketing-prerender/.test(`${scriptName} ${command}`)) {
        failures.push(`${relativePath} contains a release-excluded command in script ${scriptName}`);
      }
      for (const match of command.matchAll(/--filter(?:=|\s+)([^\s;&|]+)/gu)) {
        if (!workspaceNames.has(match[1])) {
          failures.push(`${relativePath} script ${scriptName} targets missing workspace package: ${match[1]}`);
        }
      }
      for (const match of command.matchAll(/\b(?:bash|node)\s+(?!-)([^\s;&|]+)/gu)) {
        const commandPath = match[1].replace(/^['"]|['"]$/gu, '');
        if (commandPath.includes('$') || commandPath.startsWith('node:')) continue;
        const resolved = path.resolve(destination, directory === '.' ? '' : directory, commandPath);
        const resolvedRelative = normalizePath(path.relative(destination, resolved));
        if (
          !commandPath.startsWith('dist/') &&
          (!resolved.startsWith(`${destination}${path.sep}`) ||
            !(await verificationPathExists(resolvedRelative)))
        ) {
          failures.push(`${relativePath} script ${scriptName} references missing command path: ${commandPath}`);
        }
      }
    }
  }
...
async function verifyRepositoryMetadata() {
  ...
  if (
    ...
    (!gitAttributes.includes('* text=auto eol=lf') ||
      !gitAttributes.includes('*.png binary'))
  ) {
    failures.push('.gitattributes must preserve manifest-stable text and binary files');
  }
  ...
  if (rootPackage.version !== verifiedManifest.releaseTag?.slice(1)) {
    failures.push('package.json version does not match PUBLIC_MANIFEST.json releaseTag');
  }
  if (rootPackage.repository?.url !== `${verifiedManifest.repository}.git`) {
    failures.push('package.json repository does not match PUBLIC_MANIFEST.json repository');
  }
  if (rootPackage.homepage !== `${verifiedManifest.repository}#readme`) {
    failures.push('package.json homepage does not match PUBLIC_MANIFEST.json repository');
  }
  if (rootPackage.bugs?.url !== `${verifiedManifest.repository}/issues`) {
    failures.push('package.json bugs URL does not match PUBLIC_MANIFEST.json repository');
  }
}
...
async function verifyCommunityTypeVocabulary() {
  ...
  const sdkContracts = await readTextIfPresent('packages/sdk-js/src/contracts.ts');
  if (sdkContracts) {
  ...
  if (serverModels && sdkTypes) {
    ...
    for (const auditField of [
      ...
      if (!sdkTypes.includes(`${auditField}:`)) {
        failures.push(`Community SDK AuditEvent omits server evidence field ${auditField}`);
      }
    }
  }
...
async function verifyDockerAndWorkflow() {
  ...
  if (dockerfile) {
    ...
    if (!dockerfile.includes('examples/chatgpt-tunnel')) {
      failures.push('Dockerfile does not package the ChatGPT tunnel configuration');
    }
  }
  ...
  if (workflow) {
    ...
    if (boundaryIndex === -1) {
      failures.push('Public workflow does not scan the tracked checkout boundary');
    } else if (attestationIndex === -1 || boundaryIndex < attestationIndex) {
      failures.push('Public workflow must attest before scanning the tracked checkout boundary');
    }
    if (!workflow.includes('corepack pnpm test:e2e:community')) {
      failures.push('Public workflow does not run generated-tree Community Playwright');
    }
    if (!workflow.includes('corepack pnpm test:postgres:no-skip')) {
      failures.push('Public workflow does not enforce the zero-skip Postgres release suite');
    }
    if (!/node-version:\s*\$\{\{ matrix\.node-version \}\}/u.test(workflow) || !workflow.includes("['22', '24']")) {
    ...
    if (!codeqlJob) {
      ...
      if (!codeqlJob.includes('security-events: write')) {
        failures.push('Public CodeQL job does not grant security-events: write');
      }
    }
    if (!secretScanJob) {
...
async function verifyDocumentation() {
  ...
  for (const relativePath of await getVerificationFiles()) {
    ...
    const body = await fs.readFile(path.join(destination, relativePath), 'utf8');
    if (rootPackage) {
      for (const match of body.matchAll(/corepack pnpm ([a-z][a-z0-9:-]*)/gu)) {
        if (!['audit', 'dlx', 'exec', 'install'].includes(match[1]) && typeof rootPackage.scripts?.[match[1]] !== 'string') {
          failures.push(`${relativePath} documents missing package script: ${match[1]}`);
        }
      }
    }
    for (const match of body.matchAll(/\]\(([^)]+)\)/gu)) {
      ...
      let target = (hashIndex === -1 ? linkTarget : linkTarget.slice(0, hashIndex))
        .split('?', 1)[0];
      try {
        target = decodeURIComponent(target);
        fragment = decodeURIComponent(fragment);
      } catch {
        failures.push(`Invalid encoded Markdown link in ${relativePath}: ${match[1]}`);
        continue;
      }
      ...
      if (
        ...
        failures.push(`Dangling relative Markdown link in ${relativePath}: ${match[1]}`);
      } else if (
        fragment &&
        path.extname(resolvedRelative).toLowerCase() === '.md' &&
        !(await markdownHasAnchor(resolvedRelative, fragment))
      ) {
        failures.push(`Dangling Markdown heading link in ${relativePath}: ${match[1]}`);
      }
    }
  }
}
...
async function verifyStrictBoundary() {
  for (const relativePath of await getVerificationFiles()) {
    ...
    if (!['.css', '.json', '.md', '.mjs', '.sh', '.ts', '.tsx', '.yaml', '.yml'].includes(extension) && !relativePath.endsWith('.env.example')) continue;
    const body = await fs.readFile(path.join(destination, relativePath), 'utf8');
    for (const [pattern, reason] of forbiddenContentRules) {
      ...
      pattern.lastIndex = 0;
      if (pattern.test(boundaryBody)) failures.push(`Forbidden content in ${relativePath}: ${reason}`);
    }
    if (
      relativePath.startsWith('apps/server/') &&
      /globalThis\.fetch\s*=/u.test(body)
    ) {
      failures.push(`Forbidden content in ${relativePath}: server code replaces globalThis.fetch`);
    }
    if (extension === '.md' || relativePath.endsWith('.env.example')) {
      for (const [pattern, reason] of narrativeRules) {
        pattern.lastIndex = 0;
        if (pattern.test(body)) failures.push(`Public narrative copy in ${relativePath}: ${reason}`);
      }
      const positiveClaims = body.replace(/\bnot\s+(?:currently\s+)?(?:published|available)\s+(?:to|on|from|via)\s+(?:the\s+)?npm(?:\s+registry)?\b/giu, '');
      if (/\b(?:available|published|installable)\s+(?:on|from|via|to)\s+(?:the\s+)?npm(?:\s+registry)?\b/iu.test(positiveClaims)) {
        failures.push(`Public narrative copy in ${relativePath}: unpublished npm distribution claim`);
      }
      if (/\b(?:ghcr\.io|docker\.io|quay\.io)\/[^\s`'"]*actionproxy\b/iu.test(body)) {
        failures.push(`Public narrative copy in ${relativePath}: unpublished registry-image claim`);
      }
    }
  }
}
...
async function verifySecrets() {
  ...
  if (!(await verificationPathExists('scripts/scan-public-secrets.mjs'))) return;
...
async function initializeCheckoutVerification() {
  ...
  if (canonicalDestination !== canonicalTopLevel) {
    fail(`Checkout verification must run at the Git worktree root: ${canonicalTopLevel}`);
  }

  const trackedPaths = runGit(['ls-files', '-z', '--cached'])
    .split('\0')
    .filter(Boolean)
    .sort(comparePaths);
  for (const relativePath of trackedPaths) {
    if (!isSafeRelativePath(relativePath)) {
      fail(`Git reported an unsafe tracked path: ${relativePath}`);
    }
    try {
      const stat = await fs.lstat(path.join(destination, relativePath));
      if (stat.isSymbolicLink() || !stat.isFile()) {
        failures.push(`Tracked public path is not a regular file: ${relativePath}`);
      }
    } catch (error) {
      failures.push(
...
async function getVerificationFiles() {
  ...
  const files = [];
  for await (const relativePath of walkAllFiles(destination)) files.push(relativePath);
...
async function* walkAllFiles(directory, base = directory) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isSymbolicLink()) {
      failures.push(`Symbolic link is not allowed in public artifact: ${relativePath}`);
...
async function readTextIfPresent(relativePath) {
  ...
  return (await exists(absolutePath)) ? fs.readFile(absolutePath, 'utf8') : undefined;
}
...
function isSemverTag(value) {
  ...
  return Boolean(match) && !match?.[1]?.split('.').some((part) => /^\d+$/u.test(part) && part.length > 1 && part.startsWith('0'));
}
